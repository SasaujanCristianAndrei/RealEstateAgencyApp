...
$query = "SELECT MIN(pret) AS PretMinim ,MAX(pret) AS PretMaxim,AVG(pret) AS PretMediu,vanzare,moneda
FROM Oferta o JOIN Spatiu s ON(o.id_spatiu=s.id_spatiu) JOIN Tip t ON(s.id_tip=t.id_tip)
WHERE t.denumire='".$tip."'
GROUP BY vanzare,moneda";
...
for ($i=0; $i < $num_results; $i++)
{
	...
	echo '<td align="center">'.htmlspecialchars(stripslashes($row['PretMinim'])).'</td>';
	echo '<td align="center">'.stripslashes($row['PretMaxim']).'</td>';
	echo '<td align="center">'.stripslashes($row['PretMediu']).'</td>';
